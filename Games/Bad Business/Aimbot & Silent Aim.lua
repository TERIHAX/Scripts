...
if getgenv().Aimbot_Settings.Ignore_Walls then--wallbang without loop O_o
...
getgenv().Targets={}--targeting system
if getgenv().Aimbot_Settings.Target=="All"then--if we target all then add everypart to list
    getgenv().Targets={"LeftHand","Abdomen","RightLeg","Hips","RightFoot","LeftArm","LeftForearm","RightForearm","RightForeleg","RightHand","Chest","RightArm","Neck","Head","LeftForeleg","LeftLeg","LeftFoot"}
elseif getgenv().Aimbot_Settings.Target=="Ends"then--if edges then we add every end to list
    getgenv().Targets={"LeftHand","RightFoot","RightHand","Head","LeftFoot"}
else--or we add itself
    getgenv().Targets={getgenv().Aimbot_Settings.Target}
end
if getgenv().AimHack then return else getgenv().AimHack="Shoote Merthod fOR bAd Business!??!?!"end--we allow the prevoiuse code to be rexxecuted because it needs to be to update settings
...
game.RunService.RenderStepped:Connect(function()
    target=nil
    if getgenv().Aimbot_Settings.Enabled and IsAlive()then
        UpdateClientTeam()
        local a=math.huge
        for _,v in pairs(GetEnemys())do
            for _,c in pairs(getgenv().Targets)do
                local main=GetHB(v.Hitbox,c)
                if getgenv().Aimbot_Settings.Ignore_Fov then
                    if(main.Position-PlayerTable[game.Players.LocalPlayer].Root.Position).Magnitude<a and(CanHit(main.Position,{},NotIgnore,true)or getgenv().Aimbot_Settings.Ignore_Walls)then
                        target=main
                    ...
                    if(Vector2.new(partloc.X,partloc.Y)-Vector2.new(center.X,center.Y)).Magnitude<=getgenv().Aimbot_Settings.Fov then
                        if(main.Position-PlayerTable[game.Players.LocalPlayer].Root.Position).Magnitude<a and(CanHit(main.Position,{},NotIgnore,true)or getgenv().Aimbot_Settings.Ignore_Walls)then
                            target=main
                            a=(main.Position-PlayerTable[game.Players.LocalPlayer].Root.Position).Magnitude
                        end
        ...
        if getgenv().Aimbot_Settings.Silent and target then
            game.Workspace.CurrentCamera.CFrame=CFrame.new(game.Workspace.CurrentCamera.CFrame.Position,target.Position)
        end
        if target and(Rmb or getgenv().Aimbot_Settings.Auto_Aim)and getgenv().Aimbot_Settings.Auto_Shoot and game.Workspace.CurrentCamera.CFrame==CFrame.new(game.Workspace.CurrentCamera.CFrame.Position,target.Position)then
            if getgenv().Aimbot_Settings.Auto_Aim then
        ...
        if getgenv().Aimbot_Settings.Silent then
...
coroutine.wrap(function()
    while game.RunService.RenderStepped:Wait()do
        pcall(function()
            if getgenv().Aimbot_Settings.Enabled and target and getgenv().Aimbot_Settings.Auto_Aim and IsAlive()then
                repeat game.RunService.RenderStepped:Wait()
                until not target or not getgenv().Aimbot_Settings.Enabled or not getgenv().Aimbot_Settings.Auto_Aim
                if getgenv().Aimbot_Settings.Aim_Out_Delay~=0 then
                    wait(getgenv().Aimbot_Settings.Aim_Out_Delay)
                end
                if(not target or not getgenv().Aimbot_Settings.Enabled or not getgenv().Aimbot_Settings.Auto_Aim)and not Rmb then
                    require(game.ReplicatedStorage.TS).Input:AutomateEnded("Aim")
                end
                if(not target or not getgenv().Aimbot_Settings.Enabled or not getgenv().Aimbot_Settings.Auto_Aim)and shift then
...
coroutine.wrap(function()--made by The3Bakers#4565
--discord link https://discord.gg/vQQqcgBWCG
_G.Aimbot_Settings={
    Enabled=true,--true,false  set to true to enable aimbot set to false to disable
    Silent=false,--true,false  set to true to enable silent aim mode, set to false to disable silent aim
...
--setclipboard("https://discord.gg/vQQqcgBWCG")
    ...
    while wait(1)do
        pcall(function()--epic fov circle
            if getgenv().Aimbot_Settings.Enabled and getgenv().Aimbot_Settings.Show_Fov and not getgenv().Aimbot_Settings.Ignore_Fov then
                local center=game.Workspace.CurrentCamera:WorldToViewportPoint(game.Workspace.CurrentCamera.CFrame.Position+game.Workspace.CurrentCamera.CFrame.LookVector)
                local a=Drawing.new("Circle")
                a.Visible=true
                a.Position=Vector2.new(center.X,center.Y)
                a.Color=getgenv().Aimbot_Settings.Fov_Color
                a.Thickness=2.5
                a.Transparency=1
                a.NumSides=100
                a.Radius=getgenv().Aimbot_Settings.Fov
                coroutine.wrap(function()
                    wait(1.1)
                    a:Remove()
                end)()
...
setreadonly(mt,false)
mt.__newindex=newcclosure(function(obj,prop,newprop)
    if not getgenv().Aimbot_Settings.Silent and target and getgenv().Aimbot_Settings.Enabled and(Rmb or getgenv().Aimbot_Settings.Auto_Aim)and tostring(obj)=="Camera"and prop=="CFrame"then
        newprop=CFrame.new(newprop.Position,target.Position)--arms get stuck behind if we use regular renderstep
    end
    return oldnewIndex(obj,prop,newprop)
end)
